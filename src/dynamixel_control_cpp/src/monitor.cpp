...
std::string bar(double percent, int width = 30) {
    ...
    for (int i = 0; i < width; i++) {
        if (i < filled) {
            if (percent > 75) result += "\033[31m#\033[0m";       // rojo
            else if (percent > 40) result += "\033[33m#\033[0m";  // amarillo
            else result += "\033[32m#\033[0m";                     // verde
        } else {
...
int main()
{
    ...
    std::cout << "\n  Monitor Dynamixel - Presiona Ctrl+C para salir\n" << std::endl;

    while (running) {
        for (auto& s : servos)
            read_servo(packetHandler, portHandler, s);

        // Mover cursor arriba para sobreescribir (3 líneas header + 5 líneas por servo + separadores)
        std::cout << "\033[" << (3 + servos.size() * 6 + 1) << "A";

        std::cout << "\033[36m  ╔══════════════════════════════════════════════════════════════╗\033[0m" << std::endl;
        std::cout << "\033[36m  ║        MONITOR DYNAMIXEL EN TIEMPO REAL                     ║\033[0m" << std::endl;
        std::cout << "\033[36m  ╚══════════════════════════════════════════════════════════════╝\033[0m" << std::endl;

        for (auto& s : servos) {
            double lp = s.load_percent();

            std::cout << "  \033[1;37m─── Motor ID " << std::setw(2) << s.id << " ───────────────────────────────────────────────\033[0m" << std::endl;
            ...
            std::cout << "    Load: " << std::setw(5) << std::setprecision(1) << lp << "% "
                      << s.load_dir() << " [" << bar(lp) << "] ";
            if (lp > 75) std::cout << "\033[31m ALTA!\033[0m";
            else if (lp > 40) std::cout << "\033[33m media\033[0m";
            else std::cout << "\033[32m baja \033[0m";
            std::cout << "     " << std::endl;
